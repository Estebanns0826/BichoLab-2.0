...
############ TOCA METER ESTO A LA APP PARA INGRESAR AL FORO ##############
    def forum_post(e):
        ...
        # Función para mostrar la ventana "Estoy en blanco"
        '''def show_blank_page(e):
            page.clean()
            page.add(ft.Text("Estoy en blanco", size=30, weight=ft.FontWeight.BOLD, color="black"))
            page.update()

        # Crear el botón de "Publicar en el foro"
        boton_publicar = ft.ElevatedButton(
            text="Publicar en el foro",
            on_click=show_blank_page
        )

        # Crear el botón de "Ir Atrás"
        boton_atras = ft.ElevatedButton(
            text="Ir Atrás",
            on_click=lambda e: show_image_upload_screen(e)  # Llama a la función al hacer clic
        )

        # Agregar el título, el icono, el botón de publicar, los posts y el botón de "Ir Atrás" a la página
        page.add(
            ft.Container(
                content=ft.Column(
                    [
                        ft.Container(
                            content=ft.Text("Bienvenido al BichoForo", size=30, weight=ft.FontWeight.BOLD, color="black"),
                            alignment=ft.alignment.center
                        ),
                        icono,
                        boton_publicar,  # Mover el botón de "Publicar en el foro" antes de los posts
                        ft.Container(
                            content=ft.Column(post_elementos, spacing=5),
                            alignment=ft.alignment.center
                        ),
                        boton_atras  # Agregar el botón de "Ir Atrás"
                    ],
                    alignment=ft.MainAxisAlignment.CENTER,
                    horizontal_alignment=ft.CrossAxisAlignment.CENTER,
                    spacing=10,
